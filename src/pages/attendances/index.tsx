...
import { useRoqClient, useAttendanceFindManyWithCount } from 'lib/roq';
...
export function AttendanceListPage(props: AttendanceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useAttendanceFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.attendance.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      cell: ({ row: { original: record } }: any) => (record?.check_in ? format(record?.check_in, 'dd-MM-yyyy') : ''),
    },
    {
      id: 'check_out',
      header: 'Check Out',
      accessorKey: 'check_out',
      cell: ({ row: { original: record } }: any) => (record?.check_out ? format(record?.check_out, 'dd-MM-yyyy') : ''),
    },
